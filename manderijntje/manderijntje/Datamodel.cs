...
namespace manderijntje
{
    ...
    public class Link
    {
        ...
        private void addTime(Link link, DateTime start, DateTime end, int delay)
        {
            link.times.Add(start);
            for (DateTime i = start; i <= end; start.AddMinutes(delay))
            {
                link.times.Add(i);
            }
        }
        ...
        private TimeSpan GetWeight(Node start, Node end)
        {
            ...
            double KmY = dY * (111.320 * Math.Cos(deg2rad(Start.x)));
